...
## 2026-06-09 - Clarify consumer service drops and voltage-drop units

- What was decided: Consumer service-drop installation now uses explicit buildings_tem connection_point to vertice_id pairs, rather than running a Dijkstra path lookup per consumer to rediscover the upstream endpoint. Cable material accounting was renamed to material_length_by_cable_km, and cluster logs now report material_length. Voltage-drop sizing now has separate helper methods for total percentage budgets and percentage-per-km consumer budgets.
- Why: The database already stores service-drop endpoints directly, material length includes parallel cables while physical line length does not, and consumer percentage-per-km settings need to be converted to a total line budget based on service-drop length. This makes endpoint, length, and voltage-drop semantics visible in code and tests.
- What was rejected and why: Keeping per-consumer get_path_to_bus calls was rejected because it is redundant for service drops and obscures the endpoint contract. Renaming or changing lines_result.length_km was rejected because rows should continue to store physical segment length while parallel stores cable count. Reusing one impedance helper for both feeder and consumer sizing was rejected because the two call sites use different voltage-drop units.

## 2026-06-09 - Separate MV-direct load filtering from service-drop voltage drop

- What was decided: Replace the hard-coded 100 kW Commercial/Public LV exclusion with configurable MV_DIRECT_CONNECTION_LOAD_THRESHOLD_KW, and collapse consumer service-drop voltage-drop settings into one VOLTAGE_DROP_LOAD_PERCENT_PER_KM for all modeled consumer connections.
- Why: Large Commercial/Public consumers are plausibly supplied through dedicated MV-side infrastructure depending on DSO practice, but the threshold should be user-configurable. The previous small/large consumer voltage-drop split added complexity without enough value for this tool.
